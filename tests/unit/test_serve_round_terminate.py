...
def test_terminate_round_should_wait_with_base_grace_when_no_extra_grace_given(monkeypatch):
    waits: list[int | None] = []
    ...
    _serve_round._terminate_round(_Proc())

    assert waits == [_serve_round._ROUND_TERM_GRACE_S]


def test_terminate_round_should_widen_wait_by_extra_grace_when_given(monkeypatch):
    waits: list[int | None] = []
    ...
    _serve_round._terminate_round(_Proc(), extra_grace_s=45)

    assert waits == [_serve_round._ROUND_TERM_GRACE_S + 45]


def test_terminate_round_should_treat_zero_extra_grace_as_no_op_when_given(monkeypatch):
    waits: list[int | None] = []
    ...
    _serve_round._terminate_round(_Proc(), extra_grace_s=0)
