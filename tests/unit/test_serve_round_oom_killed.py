...
def test_round_oom_killed_should_emit_pointer_only_event_when_oom_kill_rises(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(_serve_cgroup.metrics, "cgroup_memory_usage", lambda **k: _usage(4, oom=1))

    cur, baseline = _serve_cgroup._emit_round_cgroup_memory(tmp_path, log, 9)
    ...
    _serve_cgroup._maybe_emit_oom_killed(tmp_path, log, 9, cur, 137, baseline)
...
def test_round_oom_killed_should_not_emit_when_sibling_oom_kill_but_clean_exit(
    ...
    _serve_cgroup._ROUND_CGROUP_STATE_BY_LOG_DIR[tmp_path] = _cgroup_state(oom_kill=3)
    monkeypatch.setattr(_serve_cgroup.metrics, "cgroup_memory_usage", lambda **k: _usage(4))

    cur, baseline = _serve_cgroup._emit_round_cgroup_memory(tmp_path, log, 9)
    # clean exit, not a kill
    _serve_cgroup._maybe_emit_oom_killed(tmp_path, log, 9, cur, 0, baseline)
...
def test_round_oom_killed_should_not_emit_when_oom_kill_counter_unchanged(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(_serve_cgroup.metrics, "cgroup_memory_usage", lambda **k: _usage(2))

    cur, baseline = _serve_cgroup._emit_round_cgroup_memory(tmp_path, log, 1)
    _serve_cgroup._maybe_emit_oom_killed(tmp_path, log, 1, cur, 0, baseline)
...
def test_round_oom_killed_should_not_emit_when_supervisor_mem_terminated_the_round(
    ...
    _serve_cgroup._ROUND_CGROUP_STATE_BY_LOG_DIR[tmp_path] = _cgroup_state(oom_kill=1)
    monkeypatch.setattr(_serve_cgroup.metrics, "cgroup_memory_usage", lambda **k: _usage(1))
    ...
    _serve_cgroup._maybe_emit_oom_killed(tmp_path, log, 1, cur, -15, baseline)
...
def test_post_round_verdicts_should_be_unchanged_when_oom_killed_fires(tmp_path, monkeypatch):
    ...
    def _run(log_dir, oom_kill_baseline, oom_kill_now):
        ...
        if oom_kill_baseline is not None:
            _serve_cgroup._ROUND_CGROUP_STATE_BY_LOG_DIR[log_dir] = _cgroup_state(
                oom_kill=oom_kill_baseline
            )
            monkeypatch.setattr(
                _serve_cgroup.metrics, "cgroup_memory_usage", lambda **k: _usage(oom_kill_now)
            )
        else:
