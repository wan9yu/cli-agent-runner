...
def test_manifest_sigterm_cooperative_should_return_false_when_binary_is_none():
    from agent_runner._plugin_manifest import manifest_sigterm_cooperative

    assert manifest_sigterm_cooperative(None) is False


def test_manifest_sigterm_cooperative_should_return_false_when_binary_unregistered():
    from agent_runner._plugin_manifest import manifest_sigterm_cooperative

    assert manifest_sigterm_cooperative("nonexistent_binary") is False


def test_manifest_sigterm_cooperative_should_match_registered_manifest_flag():
    from agent_runner._plugin_manifest import (
        PluginManifest,
        manifest_sigterm_cooperative,
        register_manifest,
    )

    register_manifest(PluginManifest(name="cooperative_binary", sigterm_cooperative=True))

    assert manifest_sigterm_cooperative("cooperative_binary") is True


def test_manifest_sigterm_cooperative_should_return_false_for_claude_name_binary_mismatch():
    """claude_rate_limit's manifest name ("claude_rate_limit") is historically
    decoupled from the "claude" binary it targets -- a real manifest IS
    registered, just under a different name, so an unmatched binary reads as
    non-cooperative by omission, never because a manifest was found and read
    as False."""
    from agent_runner._plugin_manifest import (
        PluginManifest,
        manifest_sigterm_cooperative,
    ...
    register_manifest(PluginManifest(name="claude_rate_limit", sigterm_cooperative=False))

    assert manifest_sigterm_cooperative("claude") is False
