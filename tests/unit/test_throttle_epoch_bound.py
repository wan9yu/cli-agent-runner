...
def test_coerce_epoch_rejects_far_future(monkeypatch):
    now = 1_700_000_000.0
...
def test_coerce_epoch_rejects_huge_digit_string():
...
def test_active_throttles_does_not_park_forever_on_poisoned_epoch(tmp_path: Path) -> None:
...
def test_check_throttle_state_does_not_crash_on_poisoned_epoch(tmp_path: Path) -> None:
    clock = FakeClock(epoch=1_700_000_000.0)
    evs = [
        {
            "event": "transient_error_detected",
