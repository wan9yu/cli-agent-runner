...
def _run(tmp_path, events, result=None, agent_name="pi"):
    """Write a round log, run the detector; return (usage, transient, auth) mocks."""
...
@pytest.mark.parametrize(
    ("error_text", "classification"),
    [
        (_ERR_429, "rate_limit_model"),
        (_ERR_503, "api_transient_5xx"),
...
def test_given_exhausted_retries_when_after_round_then_transient_only(
    ...
    usage_emit, err_emit, auth_emit = _run(
        ...
        [
            ...
            {"type": "auto_retry_end", "success": False, "attempt": 3, "finalError": error_text},
            _AGENT_SETTLED,
        ],
    )
...
def test_given_auth_failure_when_after_round_then_auth_event_and_no_transient(tmp_path):
    ...
    kw = auth_emit.call_args.kwargs
    assert kw["agent"] == "pi"
    assert kw["round_num"] == 1
