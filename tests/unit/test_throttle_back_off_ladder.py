...
def test_backoff_exponent_should_reset_when_success_recorded(tmp_path: Path) -> None:
    _write(
        tmp_path,
        [_detected("claude", 2_000), _detected("claude", 2_100)],
    )
    assert _throttle._backoff_exponent(tmp_path, "claude") == 1  # built up before the reset
...
def test_active_throttles_should_extend_reset_when_backoff_exponent_grows(tmp_path: Path) -> None:
...
def test_active_throttles_should_use_verbatim_reset_epoch_when_classification_is_rate_limit_account(
    tmp_path: Path,
) -> None:
    clock = FakeClock(epoch=1_000)
    _write(
