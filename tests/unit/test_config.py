...
def test_default_runtime_should_have_wrapup_grace_s_zero_when_loaded(
    tmp_path: Path,
) -> None:
    """0 = disabled -- byte-identical grace behavior unless an operator opts in."""
    (tmp_path / "prompt.md").write_text("p")
    (tmp_path / "agent-runner.toml").write_text(
        "schema_version = 1\n"
        "[agent]\n"
    ...
    assert cfg.runtime.wrapup_grace_s == 0


def test_explicit_wrapup_grace_s_should_round_trip_when_loaded(
    tmp_path: Path,
) -> None:
    (tmp_path / "prompt.md").write_text("p")
    (tmp_path / "agent-runner.toml").write_text(
    ...
    assert cfg.runtime.wrapup_grace_s == 30


def test_negative_wrapup_grace_s_should_be_rejected_when_loaded(
    tmp_path: Path,
) -> None:
    """Negative can never reach _terminate_round -- the config layer alone
    makes "wrapup_grace_s only ever lengthens" a static guarantee."""
    (tmp_path / "prompt.md").write_text("p")
    (tmp_path / "agent-runner.toml").write_text(
        "schema_version = 1\n"
        "[agent]\n"
    ...
    with pytest.raises(ValueError, match="wrapup_grace_s: must be >= 0"):
        load_config(tmp_path / "agent-runner.toml")
