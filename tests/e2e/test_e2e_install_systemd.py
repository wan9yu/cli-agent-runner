...
def test_given_pi_install_uninstall_lifecycle(
    ...
    install = f"cd {pi_workdir} && {pi_install_agent_runner} --config {pi_config} install"
    ...
    _ssh(
        f"cd {pi_workdir} && {pi_install_agent_runner} --config {pi_config} uninstall", check=False
    )
