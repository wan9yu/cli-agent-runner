...
def test_brake_and_nudge_should_default_off() -> None:
...
def test_brake_floor_and_engage_guard_should_be_pinned() -> None:
    assert metrics._MIN_MEMORY_HIGH_BYTES == 64 * 1024 * 1024
    assert metrics._MIN_BRAKE_CURRENT_BYTES == 128 * 1024 * 1024


def test_brake_step_pct_should_be_boot_capped_at_50() -> None:
    from agent_runner.config.parsers import _MAX_BRAKE_STEP_PCT

    assert _MAX_BRAKE_STEP_PCT == 50


def test_write_helpers_should_only_target_the_resolved_leaf_never_an_ancestor() -> None:
...
def test_nudge_path_should_never_call_killpg() -> None:
