from __future__ import annotations

from tests.literate.parser import LiterateBlock, parse_literate_blocks  # noqa: F401


def test_md_with_single_bash_block_should_return_one_block_when_parsed() -> None:
    md = "intro\n\n```bash\necho hi\n```\n"

    blocks = parse_literate_blocks(md)

    assert len(blocks) == 1
    assert blocks[0].code.strip() == "echo hi"
    assert blocks[0].expected_substring is None
    assert blocks[0].expected_status == 0
    assert blocks[0].skip is False
    assert blocks[0].env == {}


def test_md_with_assert_marker_should_record_substring_when_parsed() -> None:
    md = "```bash\necho hello\n```\n<!-- assert: hello -->\n"

    [block] = parse_literate_blocks(md)

    assert block.expected_substring == "hello"


def test_md_with_status_marker_should_record_status_when_parsed() -> None:
    md = "```bash\nfalse\n```\n<!-- assert-status: 1 -->\n"

    [block] = parse_literate_blocks(md)

    assert block.expected_status == 1


def test_md_with_skip_marker_should_mark_block_skipped_when_parsed() -> None:
    md = "```bash\nclaude /login\n```\n<!-- skip-test -->\n"

    [block] = parse_literate_blocks(md)

    assert block.skip is True
    assert block.skip_reason is None


def test_md_with_skip_reason_should_record_reason_when_parsed() -> None:
    md = "```bash\nclaude /login\n```\n<!-- skip-test: needs interactive OAuth -->\n"

    [block] = parse_literate_blocks(md)

    assert block.skip is True
    assert block.skip_reason == "needs interactive OAuth"


def test_md_with_env_marker_should_record_env_when_parsed() -> None:
    md = "```bash\necho $X\n```\n<!-- env: X=hi -->\n<!-- env: Y=bye -->\n"

    [block] = parse_literate_blocks(md)

    assert block.env == {"X": "hi", "Y": "bye"}


def test_non_bash_codeblock_should_be_skipped_when_parsed() -> None:
    md = "```python\nprint('x')\n```\n```bash\ntrue\n```\n"

    blocks = parse_literate_blocks(md)

    assert len(blocks) == 1
    assert blocks[0].code.strip() == "true"


def test_two_bash_blocks_should_keep_order_when_parsed() -> None:
    md = "```bash\necho one\n```\n\n```bash\necho two\n```\n"

    blocks = parse_literate_blocks(md)

    assert [b.code.strip() for b in blocks] == ["echo one", "echo two"]


def test_blank_line_between_markers_should_end_marker_run_when_parsed() -> None:
    """Markers must be contiguous; a blank line ends the marker run for that block.

    Without this rule, a visual separator after a block's markers would silently
    bind the next markers (intended for a later block) to this one.
    """
    md = (
        "```bash\necho first\n```\n"
        "<!-- assert: first -->\n"
        "\n"  # blank line — ends marker run for the first block
        "<!-- assert: second -->\n"  # NOT attached to the first block
    )

    [block] = parse_literate_blocks(md)

    assert block.expected_substring == "first"
