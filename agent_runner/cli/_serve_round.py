...
``_pause_poll``, ``_spawn_round``, ``post_round_verdicts``) back into its own
namespace, so ``monkeypatch.setattr("agent_runner.cli.serve_cmd.X", ...)``
and a bare call to ``X(...)`` from ``cmd()`` both keep working exactly as
before the split. The cgroup-pressure spine (round_cgroup_memory tracking,
the startup cgroup-budget probe) lives in ``_serve_cgroup.py``; this module
imports the pieces it calls directly and never redefines them.
"""
...
def post_round_verdicts(
    ...
    *,
    ...
    cur_cgroup_usage, oom_kill_baseline = _emit_round_cgroup_memory(
        log_dir, round_log_path, round_num
    )
    _maybe_emit_oom_killed(
        log_dir, round_log_path, round_num, cur_cgroup_usage, r_returncode, oom_kill_baseline
    )
