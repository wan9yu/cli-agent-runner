...
def load_and_register_plugins(plugins_cfg, log_dir=None) -> None:
    """Called from config.loader.load_config once [plugins] is parsed. For
    each discovered (name, value) NOT in plugins_cfg.disable and not already
    ...
    pyproject.toml key) — it's the identifier known BEFORE import. It is NOT
    necessarily ``PluginManifest.name`` (only known after import); for every
    builtin the two are pinned equal by
    ``test_entry_point_names_should_match_manifest_name``, but that is not
    enforced here.

    ``log_dir`` is accepted for call-site stability (config.loader threads it
    through unconditionally) though this function no longer emits anything
    itself.
    """
    import warnings
