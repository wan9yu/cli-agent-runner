...
stream-json` — the latter required for `claude` to parse
...
```bash
agent-runner install --monitor --config <toml>
```

`install` already enables, starts, and (with `--monitor`) installs the
auto-stop sidekick in one step — no separate `systemctl --user start/enable`
needed.
...
| Host memory pressure | `mem_pressure` detector + serve-loop admission gate | `host_health`'s signal ladder (PSI → swap-out rate → combined MemFree+MemAvailable low → unavailable) reports `warning` or `critical` — a bare `mem_available_mb` gate is unreliable on cache-poor small-memory hosts. PSI's critical bar is `[monitor.host_health.pressure] full_avg10_critical` (default 60.0, i.e. sustained ~60% avg10, not a single 1% blip); `some_avg10_warning` (default 5.0) gates the warning tier. `mem_pressure_gate_inert` fires once if a cache-poor-valid signal shows real pressure while the configured `[monitor.host_health.memory] avail_min_mb` (default 200) gate stays green; `mem_signal_unavailable` fires once when no tier is readable. The detector itself is notify-only (`auto_action="none"`), but `serve` separately **defers the next round** while `host_health` reports **`critical`** pressure only (`round_deferred`/`round_resumed` — a mere `warning` no longer delays a round) and, mid-round, **terminates a ballooning round** once pressure reads `critical` for `critical_consecutive_samples` (default 3) consecutive ~10s ticks in a row (`round_mem_terminated`, `round_mem_critical_sample` per tick, capped at `2 × critical_consecutive_samples` consecutive ticks — 1..6 at the default — so a sustained-critical deferred/off run doesn't write one event per tick for the whole round; any non-critical tick resets both the streak and the cap). `[monitor.host_health.pressure] in_round_terminate = false` keeps sampling and counting that streak without ever terminating a round. `[monitor.host_health.pressure] in_round_nudge = true` (default off) fires that same SIGTERM at the **first** critical sample instead of at the sustained `critical_consecutive_samples` threshold — but it is gated on `in_round_terminate = true` **and** on the round not deferring to a bounded cgroup, so enabling the nudge while `in_round_terminate = false` leaves it silently inert. Both the nudge and the sustained terminate emit `round_mem_terminated` with a `tier` field (`nudge` \| `terminate`), so the `mem_loop` give-up breaker still converges. If the **effective** cgroup budget (min finite `memory.max` and `memory.swap.max` across this process's leaf **and ancestors**, plausible vs host RAM/swap) is both-finite (`host_cgroup_memory_limit.defer` at startup), the mid-round floor defers to kernel cgroup-OOM instead (`mem_pressure_deferred_to_cgroup`) — the kernel will already contain the agent within that budget. `cgroup_path` on that boot event is the leaf; the winning cap may sit on a parent. Whole-host figures, not the agent's own share <!-- authored: default avail_min_mb / full_avg10_critical / some_avg10_warning / critical_consecutive_samples; SSOT agent_runner/config/models.py --> |
...
| A round dies to the kernel's own cgroup-OOM, not just a silent 137 | `round_oom_killed` event, folded from the same `round_cgroup_memory` scan | Emitted when `memory.events.oom_kill` rose over the round (the same delta `round_cgroup_memory` already computed — no second read). Pointer-only: `log_path`, `log_bytes`, and the oom-kill delta, never the round-log/transcript content itself. The delta rides under two field names with the identical value — `events_oom_kill_delta` is the canonical name (sibling-consistent with `round_cgroup_memory`'s `events_oom_kill_delta`) and `oom_kill_delta` is a retained legacy alias kept until a future breaking release drops it. `partial_log` reports whether the supervisor appended its own truncation trailer to the round log (a residue marker, not content) so a later reader can tell the tail is supervisor-added. The round still exits 137 and counts toward the crash streak exactly as today — this event only makes the kill legible, it never changes the give-up decision |
| Disk filling up | `disk_warning` at ≥90%, `disk_critical` at ≥95% | Sampled on `log_dir`'s partition. `disk_critical` auto-stops the service by default; `disk_warning` only alerts. Thresholds under `[monitor.host_health.disk]` (`warning_pct`/`critical_pct`) <!-- authored: disk_critical ships in the default auto_stop_on set; SSOT agent_runner/config/models.py --> |
...
| Token / cost accounting | `agent_usage_recorded` events in `events-*.jsonl` | Raw per-round records; rollups and budget alerts are the consumer's job. Emitted by the `claude` / `gemini` / `codewhale` / `pi` plugins. kimi's plugin classifies transient errors but emits no usage, because the Kimi Code CLI exposes no token counters in its stream-json output, so kimi rounds produce no usage events |
...
### Soft-brake: automated `memory.high` under sustained pressure

Distinct from the manual `MemoryHigh` unit directive below, and **opt-in**.
Set `[monitor.host_health.brake] memory_high = true` (fields in
`docs/configuration.md`'s `[monitor.host_health.brake]` table) and `serve`
reversibly writes `memory.high` on its **own** cgroup leaf (never an ancestor)
while `host_health` reports sustained pressure — at the warning threshold or
above — for `warning_consecutive_samples` ticks, then restores it on recovery
(`memory_high_released`, reason `recovered`) or unconditionally at round end
(reason `round_end`). The write is **fail-open** and **monotone** — it never
tightens an already-lower `memory.high`. It arms **only** when that leaf is
delegated to `serve`: a root system unit, or a user-mode unit
(`systemctl --user` + `loginctl enable-linger`). Otherwise it stays **inert**
and is announced as such.

`memory_high_step_pct` sets the engage value. The shipped `0` caps at current
usage (`memory.high == memory.current`) — throttle further growth with **no**
synchronous reclaim burst, the SD-backed-host-safe default. `>= 1` is opt-in
aggressive reclaim that dumps ~step% of the leaf to swap at engage (a burst
that can stall an SD-backed host's page-ins); boot-capped at 50.

Events: `memory_high_engaged`, `memory_high_released`, `memory_high_write_failed`.
`doctor` and `peek --json` report the brake state as `off` \| `armed` \|
`inert(<reason>)`.
...
with a plausible both-finite **effective** budget (min across leaf and
ancestors; `host_cgroup_memory_limit.defer`), the mid-round floor
...
| Event | Trigger | Fix |
|---|---|---|
| `config_broken` | Any `ConfigError` round exit (`78`): usually permanent startup smoke (missing/short prompt, non-git `work_dir`, agent CLI not on PATH); or, when serve is phase-aware (`skip` or a per-phase schedule), a stale `--phase`. | Battery failure: read `smoke_check_failed`, fix the config, `agent-runner start`. Stale `--phase` (no `smoke_check_failed` that round): `agent-runner restart`. |
...
The built-in `claude` plugin classifies transient errors into
4 buckets:
