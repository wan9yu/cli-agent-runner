# agent-runner thesis: in-scope and out-of-scope
...
4. Exposes a **plugin hook** (`PostRoundHook`) for extension without
   modifying core.
...
Each round emits `cost_usd` in `agent_usage_recorded`. Five lines of jq
...
> to stop rounds that exceed a spend threshold. Rejected. The event already carries the raw cost;
...
> and report templates that this project does not take on.
...
The supervisor codifies **specific scars**: 8× same tool call = stuck loop; `result` event
...
> The rolling baseline itself requires aggregation the project does not own, and the
> threshold N is project-specific. A consumer can compute this from the flat
> events file.

### Transient errors: server-authoritative vs estimated
...
- **Server-authoritative**: Anthropic's `rate_limit_event.resetsAt` is an
  exact unblock time. The supervisor respects it verbatim — no backoff multipliers, no
...
- **Estimated**: For other classifications (`rate_limit_model`,
  ...
  the same bucket again after waiting the guess, the wait increases
...
back off those estimates when they prove insufficient. It is **not**
N-σ novelty detection (which is rejected — see the section above); it
...
> Anthropic where the previous fixed 60s wait was too short — the next
...
tool of the consumer's choice. The project avoids becoming a queryable
store — that path ends with schema migrations, index management, and storage
lifecycle concerns outside this project's scope.
...
> against the events file gives the same result without shipping a query
> engine.
...
The supervisor detects (events) and provides hard-stop defenses for "continuing is harmful"
cases. It does not provide config-driven remediation chains (alert vs. stop vs.
...
> without owning webhook plumbing.

### No first-class concept of "the right way to use claude"

Core does not enforce a particular prompting style, session discipline, role
rotation pattern, or attestation scheme.

That's prompt-engineering project policy. It varies per use case and evolves
faster than a library version cycle. agent-runner is a runtime harness, not a usage
methodology.
...
from elsewhere. It does not route an agent's tool calls to a remote shell (SSH,
container, k8s pod). The `monitor --host` flag is scoped to remote
*observation*, never remote *execution*: it relays the host's event stream to a
client (`--mode events`), while detection and auto-stop stay on the host itself
(see [`runbook.md`](runbook.md) § "Remote event relay & SSH trust").
...
would be an anticipatory feature for a topology the unattended model does not use.

> **Example**: The 2026-04 zmx "ai portal" release (agent-local, shell-remote
> via a session) prompted this entry. It validates the CLI-not-MCP stance
...
- Requires owning reliability, retries, or auth for external services.
...
**Evaluation**: BI-adjacent (requires surfacing a configurable dollar
