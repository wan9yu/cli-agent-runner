...
A restart-on-exit supervisor for autonomous Claude CLI agents. Each round runs
the agent once and exits; an external service manager (systemd / launchd / bash
loop) restarts. State persists across restarts via JSON files; defenses
(timeout, process-group reap, orphan-stash, smoke-check, monitor auto-stop)
catch the recurring failure modes.
...
agent-runner 是一个面向自主运行 Claude agent 的「跑完即退、自动重启」调度框架。
