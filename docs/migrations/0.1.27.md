...
> **Forward note (added 2026-07-17 for 0.2.2)**: the "Migration from legacy
> `rate_limit_rejected` event" section below (`:96-102`) is obsolete. That alias
> was removed in **0.1.29**; it is no longer a registered event kind, so `emit()`
> raises on it and no consumer can receive it. Subscribe to
> `transient_error_detected`. The usage-aggregation guidance is likewise
> superseded — see `docs/thesis.md`, which makes aggregation an explicit non-goal.
