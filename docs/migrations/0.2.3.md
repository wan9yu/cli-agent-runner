...
that setup was already broken — silently so for a bare command found on PATH
— and the agent now runs where the config says.
...
- `pi` — Pi Coding Agent (`pi -p -na --mode json --model …`; `-na` pins pi's
  project trust off for unattended rounds). `docs/recipes/pi.md` documents the
  trust vector and why `round_timeout_s` is the only brake.
