...
The `claude_rate_limit_detector` entry-point alias (introduced 0.1.20,
kept as back-compat after the 0.1.23 rename to `claude_error_detector`)
was hard-removed in 0.1.35. The underlying class is unchanged; only the
entry-point key in `pyproject.toml` was dropped.
