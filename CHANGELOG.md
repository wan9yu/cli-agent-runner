...
### Added
- ratch `0.1.6`: `InjectedClock` extras (`strftime` / `*_ns`); `BddTestConventions(forbid_comment_labels=...)`; `ConfinedImport` for asyncio/select/pidfd (`_procwait` + `_notify`). pytest keeps BDD 6-stmt grouping and asyncio Process.kill remainder.
...
- `examples/README.zh.md` and per-directory `README.zh.md`: Chinese pages for the outer-loop examples.
...
### Added
- `host_cgroup_memory_limit` now reports whether the supervisor's own cgroup is delegated (its `cgroup.procs`/`memory.high` are writable); `doctor` shows it and advises adding `Delegate=yes` to the systemd unit when it isn't. Read-only — nothing is written to any cgroup.
- Plugin presets can declare `sigterm_cooperative` (the built-in `gemini` preset does); an observability-only declaration surfaced in `peek --json` (schema `2.3`) and `doctor` — it does not change any termination behavior.
