...
### Added
- `round_start` carries `config_digest` (sha256 of prompt files, check cmdlines, `[vcs]` dirty_action, prompt delivery, agent env). Snapshot extras (no env values) emit only when it changes. Host-health floors and give-up codes are not hashed. The kill path does not read these fields.
