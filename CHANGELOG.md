...
- Plugin presets can declare `sigterm_cooperative` (the built-in `gemini` preset does — its CLI drains and cleans up on SIGTERM); surfaced in `peek --json` (schema `2.3`) and `doctor`.
- `[runtime] wrapup_grace_s` (default 0): for an agent whose preset is `sigterm_cooperative`, lengthens the existing SIGTERM→SIGKILL grace so the CLI can wrap up on its own signal handler. It never changes *when* a round is terminated — only how long the already-decided SIGTERM waits before the SIGKILL fallback.
